...
export default {
  ...
  methods: {
    ...
    async updateTime(event, timeId, time) {
      const btn = event.target;
      const tr = btn.parentElement;
      ...
      for (let t of this.timesOnSupaJSON) {
        if (t.tutor_id == Number(tr.querySelector('#tutor').value) &&
          ...
          for (let i = 0; i <= classes; i = i + 7) {
            let dateObj = new Date(t.start_date)
            dateObj.setDate(dateObj.getDate() + i);
            let dateStr = dateObj.toISOString().slice(0, 10);
            dates.push(dateStr)
          }
          if (dates.includes(tr.querySelector('#start_date').value)) {
            console.log(dates)
            count ++
      ...
      let dates = []
      for (let i = 0; i < time.course.duration; i++) {
            let dateObj = new Date(time.start_date)
      ...
      if (count > 0) {
        ...
        tr.querySelector('#course').value = this.originalTime.course.name
        tr.querySelector('#tutor').value = this.originalTime.tutor_id
        alert (`Entry of ${tutor} (ID: ${tutorId}), teaching at ${tr.querySelector('#time').value} on ${getDayOfWeek(startDate)} starting from ${startDate}, already exists. Please choose another timeslot.`)
    ...
    <div class="timetable">
        ...
        <div>
          ...
          <form class="filter" action="">
            <p style='font-weight:bold'>Fliter by course: </p>
            <select>
        ...
        <table class="q-mt-md">
            ...
            <tr v-for="time in timesOnSupa" :key="time.id">
                ...
                <q-icon id='confirmEdit' @click='updateTime($event, time.id, time)' name="thumb_up_alt" />
